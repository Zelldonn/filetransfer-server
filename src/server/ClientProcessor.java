...
import transaction.DownloadFileHandler;
import transaction.RequestHandler;
import transaction.UploadFileHandler;
import transaction.PingHandler;
...
public class ClientProcessor extends Thread{
    ...
    ClientProcessor(Socket s) throws IOException {
        ...
        handlers.put(TransactionType.UPLOAD, new UploadFileHandler(bis, bos));

        handlers.put(TransactionType.PING, new PingHandler(bis, bos));

        handlers.put(TransactionType.FILE_INFO, new RequestHandler(bis, bos));

        handlers.put(TransactionType.DOWNLOAD, new DownloadFileHandler(bis, bos));
    }
